...
class Camera:
    def __init__(self, name):
        self.camera_mat = np.load(os.path.dirname(__file__) + './data/{}/new_camera_{}_K.npy'.format(name,name))
        self.dist_coeff = np.load(os.path.dirname(__file__) + './data/{}/new_camera_{}_D.npy'.format(name,name))
...
class BevGenerator:
    ...
    def __call__(self, front, back, left, right, car = None):
        images = [front,back,left,right]
        nomask_images = [front, back,left,right]
        ...
        mask_images = [mask((img)) 
                  for img, mask, camera in zip(images, self.masks, self.cameras)]
                    
        panaroma_images = np.hstack([nomask_images[2], nomask_images[0]])
        panaroma_images = np.hstack([panaroma_images, nomask_images[3]])
        panaroma_images = np.hstack([panaroma_images, nomask_images[1]])
        ...
        return surround, panaroma_images, mask_images

def main():
    
    cap_front = cv2.VideoCapture('/home/nam/바탕화면/CameraCalibration/sample_data/basement_0322/front.mp4')
    cap_back = cv2.VideoCapture('/home/nam/바탕화면/CameraCalibration/sample_data/basement_0322/back.mp4')
    cap_left = cv2.VideoCapture('/home/nam/바탕화면/CameraCalibration/sample_data/basement_0322/left.mp4')
    cap_right = cv2.VideoCapture('/home/nam/바탕화면/CameraCalibration/sample_data/basement_0322/right.mp4')
    ...
    while True:
        ...
        ret_front, front = cap_front.read()
        ret_back, back = cap_back.read()
        ret_left, left = cap_left.read()
        ret_right, right = cap_right.read()
        
        if not ret_front or not ret_back or not ret_left or not ret_right:
            break
